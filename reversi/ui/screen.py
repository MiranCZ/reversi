import pygame
from reversi.core.gamemanager import GameManager
import math
from reversi.ui.button import Button
from reversi.ui.bar import Bar
from reversi.ui.font.font_loader import load_font
...
game_display = pygame.display.set_mode(((pygame.display.Info().current_w, pygame.display.Info().current_h)),
                                       pygame.FULLSCREEN | pygame.SCALED)
clock = pygame.time.Clock()
pygame.display.set_caption("Reversi")
font = load_font(40)
font_stopwatch = load_font(80)
font_stopwatch_victory = load_font(60)
running = True
...
def draw_board():
    ...
    for i in range(7):
        number_y -= size / 8
        pygame.draw.line(game_display, "#7f5539", (rect_x - 4, rect_y + number_y), (rect_x + size, rect_y + number_y),
                         width=10)

    number_x = size
    for i in range(7):
        number_x -= size / 8
        pygame.draw.line(game_display, "#7f5539", (rect_x + number_x, rect_y), (rect_x + number_x, rect_y + size),
                         width=10)
...
def draw_disks():
    ...
    for row in range(8):
        for column in range(8):
            ...
            pygame.draw.circle(game_display, disk_color, (rect_x + (row * size / 8) + size / 16 + 1,
                                                          rect_y + (column * size / 8) + size / 16 + 1), 50)
...
def victory_screen():
    ...
    draw_text(f"Game took: {minutes_ten}{minutes}:{seconds_ten}{seconds}", font_stopwatch_victory, "#000000", width / 2,
              height / 2 - 30)
...
while running:
    ...
    draw_text(f"{minutes_ten}{minutes}:{seconds_ten}{seconds}", font_stopwatch, (0, 0, 0), rect_x + size + 50 + 205,
              size / 2 + 30 + 175)
